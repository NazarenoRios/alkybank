...
function useProvideAuth() {
    ...
    const signin = async (email, password) => {
        try {
            ...
            const {status, data:{accessToken}} = await axios.post(API_URL+"login", { email, password })
            if(status === 200){
            ...
            const api_user_data = await axios.get(API_URL+"me", { headers: { Authorization: `Bearer ${accessToken}` } })
